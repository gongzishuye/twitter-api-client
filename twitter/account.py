...
class Account:
    ...
    def home_latest_timeline(self, limit=math.inf, limit_datetime: datetime = None) -> list[dict]:
        return self._paginate('POST', Operation.HomeLatestTimeline, Operation.default_variables, limit, limit_datetime)

    def bookmarks(self, limit=math.inf) -> list[dict]:
        return self._paginate('GET', Operation.Bookmarks, {}, limit)

    def _paginate(self, method: str, operation: tuple, variables: dict, limit: int, limit_datetime: datetime) -> list[dict]:
        initial_data = self.gql(method, operation, variables)
        res = [initial_data]
        ids = set(find_key(initial_data, 'rest_id'))
        if len(initial_data['data']['home']['home_timeline_urt']['instructions']) > 1:
            print(initial_data['data']['home']['home_timeline_urt']['instructions'])
            raise initial_data['data']['home']['home_timeline_urt']['instructions']
        entries = initial_data['data']['home']['home_timeline_urt']['instructions'][0]['entries']
        thread_num = len(entries)
        dups = 0
        DUP_LIMIT = 3
        ...
        while (dups < DUP_LIMIT) and cursor:
            ...
            if len(data['data']['home']['home_timeline_urt']['instructions']) > 1:
                print(data['data']['home']['home_timeline_urt']['instructions'])
